...
crash dump.
            ...
            <translation>壊滅的なプール障害が発生した場合、システムの動作を制御します
この条件は、通常、基礎となるストレージデバイス（複数可）
またはプール内のすべてのデバイスの故障への接続が失われた時です
以下のようにそのようなイベントの挙動が決定されます
wait
ブロックは、デバイスの接続まで、すべてのI/Oアクセスが回収され、
エラーがクリアされます。これがデフォルトの動作です
continue
新規の書き込みI/ O要求にEIOを返しますが、
可能にするには残りの健全な装置のいずれかに読み取ります。
ディスクにコミットされるには至っていないすべての書き込み要求はブロックされます。 
panic
コンソールにプリントアウトするメッセージを出力し、システムクラッシュダンプを生成します。
</translation>
        </message>
    </context>
    <context>
...
You are about to perform the following operation(s):
            ...
            <translation>この操作は元に戻すことはできませんし、データ消失の原因となります。
あなたは以下の操作(など)を実行しようとしています:
</translation>
        </message>
        <message>
            <source>

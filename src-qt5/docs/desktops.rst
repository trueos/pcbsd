...
If you used App-Cafe to install i3 it will have automatically installed a system wide config that is the default usually generated by i3-config-wizard.  If you'd like to use Win as a modifier key and dvorak layout please run :command:`pc-extractoverlay i3dvorak`.  You may want to run :command:`pc-extractoverlay ports` if you installed using pkg directly and don't want dvorak layout.
